...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def provideOAuth1TokenSecretProvider(configuration: Configuration, clock: Clock): OAuth1TokenSecretProvider = {
    ...
    val config2 = configuration.underlying.as[JcaCookieSignerSettings]("silhouette.authenticator.cookie.signer")
    new CookieSecretProvider(settings, new JcaCookieSigner(config2), new JcaCrypter(config), clock)
  }
  ...
  def provideOAuth2StateProvider(idGenerator: IDGenerator, configuration: Configuration, clock: Clock): OAuth2StateProvider = {
    ...
    val config2 = configuration.underlying.as[JcaCookieSignerSettings]("silhouette.authenticator.cookie.signer")
    new CookieStateProvider(settings, idGenerator, new JcaCookieSigner(config2), clock)
  }
